...
class Boss:
        ...
        *,
        ...
        for iteration in range(self.max_iterations):
            ...
            decision = self._decide(
                review, iterations_left=(self.max_iterations - iteration - 1)
            )
        ...
        assert current is not None  # max_iterations >= 1 guarantees we looped once
    ...
    def _synthesize_fallback(
        ...
        the user is better served by an estimation with caveats than by zeroed
        numbers.
        """
        ...
        # Floor confidence above LOW_CONFIDENCE_THRESHOLD so the schema
        # validator that demands an "Out of scope:" prefix below the threshold
        # does not fire — that would contradict the point of returning an
        # annotated draft.
        reduced_confidence = max(LOW_CONFIDENCE_THRESHOLD + 5, last_result.confidence_pct // 2)
        new_summary = _strip_out_of_scope_prefix(new_summary)
...
def _strip_out_of_scope_prefix(summary: str) -> str:
    """The caveats block prepends a warning header; the OUT_OF_SCOPE prefix
    would no longer sit at the start of ``summary`` and the validator would
    raise. Drop any leading prefix from the original summary tail."""
    marker = OUT_OF_SCOPE_PREFIX
    if marker in summary and not summary.startswith(marker):
        return summary.replace(marker, "", 1).lstrip()
    return summary
