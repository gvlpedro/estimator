...
2. Prompt-injection heuristics (regex over known patterns).
3. PII heuristics (regex over emails, phones, IBANs).

All three raise ``InputGuardrailViolation`` (exception, never fix-and-retry).
The HTTP layer maps that to a 400 with ``{reason, message}``.
"""
...
def check_input(description: str, *, openai_client: Any | None = None) -> None:
    ...
    ``openai_client`` is optional: in tests we pass ``None`` to exercise only
    the regex layers; in production it should be set so moderation runs first.
    """
