...
def test_iterate_then_accept_runs_actor_twice_with_feedback() -> None:
    ...
    _out, trace = boss.run(actor=actor, critic=critic)
...
def test_synthesize_after_max_iterations_keeps_actor_draft_with_caveats() -> None:
    ...
    # Summary leads with the caveats block; confidence floored above 30.
    assert result.summary.startswith("⚠ Open caveats")
    assert "[critical] math_error" in result.summary
    assert result.confidence_pct >= 35
...
def test_critic_feedback_requires_blocking_issue_for_needs_iteration() -> None:
    with pytest.raises(ValueError):
        CriticFeedback(
            verdict="needs_iteration",
            issues=[
                CriticIssue(
                    ...
                    description="phase 0 looks light vs typical baselines",
                )
            ],
            confidence_in_review=50,
        )


def test_critic_feedback_reject_requires_issue() -> None:
    with pytest.raises(ValueError):
        CriticFeedback(verdict="reject", issues=[], confidence_in_review=80)
