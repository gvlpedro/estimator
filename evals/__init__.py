"""Evaluation harness for the structured estimator.
...
- ``run.py``: CLI that fires each case at one of the two modes (``actor`` →
  ``POST /api/v1/estimate``, ``acb`` → ``POST /api/v1/estimate/acb``) and
  prints a comparative table.

The pre-shipped ``golden_dataset.json`` carries 16 cases mixing project
types, scope levels, regulatory flavours, and a couple of out-of-scope
adversarials.
"""
