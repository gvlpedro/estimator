...
The point of keeping these in-tree (rather than offloading to an LLM judge)
is that the suite runs without a second LLM and tells us exactly *why* a
case failed.
"""
...
class SchemaAdherenceMetric:
    ...
    *regress* (e.g. a future change drops a validator). It also checks the
    ...
    def evaluate(self, case: GoldenCase, result: EstimationResult) -> MetricResult:
        ...
        if (
            result.confidence_pct < LOW_CONFIDENCE_THRESHOLD
            and not result.summary.startswith(OUT_OF_SCOPE_PREFIX)
        ):
            problems.append(
                f"low confidence ({result.confidence_pct}) missing "
                f"{OUT_OF_SCOPE_PREFIX!r} prefix"
            )
...
class CostBoundsMetric:
    ...
    expect cost=0/duration=1 (the placeholder envelope produced by the
    ``enforce_scope_response`` output guardrail)."""
